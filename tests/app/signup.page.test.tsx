...
describe('SignupPage', () => {
  it('renders standard signup form when no token is present', () => {
    ...
    expect(screen.getByText('Register to manage and run your academic olympiads.')).toBeInTheDocument();
    expect(screen.getByText('Sign Up as Organiser')).toBeInTheDocument();
  });

  it('renders invite claim form when token is present', () => {
    mockUseSearchParams.mockReturnValue(new URLSearchParams('?inviteToken=123'));
    render(<SignupPage />);
    expect(screen.getByText('Claim Your Account')).toBeInTheDocument();
    expect(screen.getByText('You have been invited to join an Olympiad.')).toBeInTheDocument();
