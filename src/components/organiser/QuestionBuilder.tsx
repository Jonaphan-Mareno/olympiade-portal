...
export default function QuestionBuilder({
  ...
  const updateOption = (qId: string, optIndex: number, newValue: string) => {
    ...
    if (q.type === 'single_choice' && q.correctAnswer === q.options[optIndex]) {
      ...
      newCorrect = q.correctAnswer.map((ans: string) => (ans === q.options[optIndex] ? newValue : ans));
    }
  ...
  const addPair = (qId: string) => {
    const q = questions.find((x) => x.id === qId);
    if (!q) return;
    updateQuestion(qId, { options: [...(q.options || []), { premise: '', response: '' }] });
  };

  const updatePair = (qId: string, pairIndex: number, field: 'premise' | 'response', value: string) => {
    const q = questions.find((x) => x.id === qId);
    if (!q) return;
    const newOptions = [...q.options];
  ...
  const handleTypeChange = (id: string, newType: string) => {
    ...
    updateQuestion(id, { type: newType, options: defaultOptions, correctAnswer: defaultCorrect });
  };
        ...
        <div key={q.id} className="bg-blue-50 rounded-md p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            ...
            <div className="w-full md:w-32">
              <label className="block text-sm font-semibold text-slate-900 mb-1">Marks</label>
              <input
                type="number"
                min="1"
                value={q.marks}
          ...
          <div className="mb-6">
            ...
            {imagePreviews[q.id] || q.imageUrl ? (
              <div className="mt-2 flex items-start gap-4">
                <img src={imagePreviews[q.id] || q.imageUrl!} alt="Question Preview" className="max-h-48 rounded-md border border-slate-200" />
          ...
          <div className="bg-slate-50 p-4 rounded-md border border-slate-200">
            <h5 className="text-sm font-semibold text-slate-900 mb-3">Options & Answers</h5>
              ...
              <div className="space-y-2">
                {q.options.map((opt, oIdx) => (
                  <div key={oIdx} className="flex items-center gap-3">
                    ...
                    />
                    <button type="button" onClick={() => removeOption(q.id, oIdx)} className="text-slate-400 hover:text-red-500">
                      ✕
                    </button>
                  </div>
                ))}
                <button type="button" onClick={() => addOption(q.id)} className="text-blue-600 text-sm font-medium mt-2 hover:underline">
                  + Add Option
                </button>
              </div>
            )}
            ...
            {q.type === 'true_false' && (
              <div className="space-y-3">
                <p className="text-sm text-slate-600 mb-2">Select the correct answer:</p>
                {['True', 'False'].map((opt) => (
            ...
            {q.type === 'matching' && Array.isArray(q.options) && (
              <div className="space-y-3">
                <div className="flex gap-4 mb-2">
                  <div className="flex-1 text-xs font-semibold text-slate-500 uppercase">Premise (Item)</div>
                  <div className="flex-1 text-xs font-semibold text-slate-500 uppercase">Response (Match)</div>
                ...
                {q.options.map((pair, pIdx) => (
                  <div key={pIdx} className="flex gap-4 items-center">
                    <input
                      type="text"
                      value={pair.premise}
                      onChange={(e) => updatePair(q.id, pIdx, 'premise', e.target.value)}
                    ...
                    <input
                      type="text"
                      value={pair.response}
                      onChange={(e) => updatePair(q.id, pIdx, 'response', e.target.value)}
                      placeholder="e.g. Powerhouse of the cell"
                      className="flex-1 p-2 border border-slate-300 rounded-md text-slate-900"
                    />
                    <button type="button" onClick={() => removePair(q.id, pIdx)} className="text-slate-400 hover:text-red-500">
                      ✕
                    </button>
                  </div>
                ))}
                <button type="button" onClick={() => addPair(q.id)} className="text-blue-600 text-sm font-medium mt-2 hover:underline">
                  + Add Pair
                </button>
              </div>
            )}
            ...
            {q.type === 'free_text' && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Grading Rubric / Memo (Optional)</label>
                <textarea
