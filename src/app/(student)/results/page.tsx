...
export default async function ResultsPage() {
  ...
  const schoolIds = studentMemberships.map(m => m.schoolId).filter((id): id is string => !!id);
  ...
  const [portalRows, schoolRows] = await Promise.all([
    ...
    schoolIds.length > 0 ? db.select().from(schools).where(inArray(schools.id, schoolIds)) : [],
  ]);
            ...
            return (
              <div key={m.id} className="glass-card" style={{ padding: '1.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <h3 style={{ fontSize: '1.1rem' }}>{portal?.name ?? 'Portal'}</h3>
                  <span style={{
                    fontSize: '0.75rem',
                    padding: '0.25rem 0.75rem',
                    borderRadius: 'var(--radius-sm)',
                    background: m.status === 'accepted' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(255, 255, 255, 0.05)',
                    color: m.status === 'accepted' ? 'var(--success-color)' : 'var(--text-secondary)',
                ...
                {school && (
                  <div style={{ fontSize: '0.9rem', paddingLeft: '0.75rem', borderLeft: '2px solid var(--primary-color)', color: 'var(--text-primary)' }}>
                    {school.name}
                  </div>
                )}
              </div>
